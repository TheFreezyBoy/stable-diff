...
const reactApp = prefix => {
  ...
  const generateImages = savedPrompts => new Promise(async resolve => {
    for (const savedPrompt of savedPrompts) {
      for (let i = 0; i < savedPrompt.count; ++i) {
        const positivePromptTextArea = document.querySelector('#' + prefix + '2img_prompt textarea');
        const negativePromptTextArea = document.querySelector('#' + prefix + '2img_neg_prompt textarea');
        ...
        await new Promise(async innerResolve => {
          ...
          const generateBtn = document.querySelector('#' + prefix + '2img_generate');
          const interruptBtn = document.querySelector('#' + prefix + '2img_interrupt');
  ...
  waitFor('#' + prefix + '2img_prompt_container').then(() => {
    const divElem = document.createElement('div');
    const promptContainer = document.getElementById(prefix + '2img_prompt_container');
